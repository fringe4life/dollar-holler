...
- **Bundler:** Vite 8.3.0-beta.1 for dev and production builds (Rolldown); production minify keeps `dropConsole` and sets `mangle: false` so Oxc does not rename Svelte snippet/`__name` helpers
...
- `account` - Auth provider accounts; identity is `(provider_id, account_id)` (email/password: `provider_id` = `credential`, `account_id` = user id); index `(user_id)`
...
- Better Auth is configured in `auth.server.ts` as isolate-cached `getAuth()` so `drizzleAdapter` does not read `db._` at import; UUIDv7 (uuidv7 package) for user ID generation; session cookie cache enabled. After upgrading to 1.7.3, apply `0004_purple_fat_cobra` under `src/lib/server/db/d1/` (drop `account_issuer_accountId_uidx`, then drop `issuer`). Account identity is `(provider_id, account_id)`, matching 1.6.
