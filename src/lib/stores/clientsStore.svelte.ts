...
class ClientsStore {
  ...
  async upsertClient(clientData: NewClient): Promise<string | null> {
    try {
      ...
      if (isUpdate && clientData.id) {
        ...
        if (index !== -1) {
          this.clients[index] = {
            ...
            id: clientData.id,
            updatedAt: new Date(),
            name: normalizeToNull(clientData.name),
      ...
      const isUpdate = !!clientData.id;
      const errorMessage = err instanceof Error ? err.message : `Failed to ${isUpdate ? "update" : "create"} client`;
...
export const { clients, loading, error, isLoaded, loadClients, getClientById, deleteClient, upsertClient, resetClients } = clientsStore;