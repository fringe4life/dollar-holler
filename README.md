...
- **Framework:** SvelteKit 3 (`3.0.0-next.11`) with `@sveltejs/adapter-vercel` 7 (`7.0.0-next.2`) and Svelte 5 runes (experimental `remoteFunctions`, async compiler, server instrumentation and tracing in [`vite.config.ts`](./vite.config.ts))
...
The application is configured for Vercel deployment with the Vercel adapter. [`vercel.json`](./vercel.json) sets `installCommand` to `bun install --ignore-scripts` and `buildCommand` to `svelte-kit sync && panda build && bun run build` (aligns with Panda static CSS output on the platform).
