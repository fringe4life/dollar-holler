...
async function main() {
  ...
  if (users.length === 0) {
    throw new Error('No auth users found. Create at least 3 users via Better Auth first.')
  }
  
  if (users.length < 3) {
    throw new Error(`Found only ${users.length} users. Need at least 3 users for proper data distribution.`)
  }

  console.log(`Found ${users.length} users. Distributing data randomly among them.`)
  ...
  const settingsData: NewSettings[] = users.map(u => ({
    ...
    city: ['Coolville', 'Tech City', 'Dev Town', 'Code Springs'][Math.floor(Math.random() * 4)],
  ...
  const clientsData: NewClient[] = clientNames.map(name => ({
    ...
    city: ['Somewhereville', 'Nowhere', 'Anywhere', 'Tech City'][Math.floor(Math.random() * 4)],
    state: ['TN', 'CA', 'NY', 'FL'][Math.floor(Math.random() * 4)],
    zip: `${Math.floor(Math.random() * 90000) + 10000}`,
    clientStatus: (Math.random() > 0.2 ? 'active' : 'archive') as 'active' | 'archive',
  }))
  ...
  for (const client of insertedClients) {
    ...
    for (let i = 0; i < invoiceCount; i++) {
      ...
      invoicesData.push({
        ...
        subject: invoiceSubjects[Math.floor(Math.random() * invoiceSubjects.length)],
        issueDate,
        dueDate,
        ...
        invoiceStatus: (['draft', 'sent', 'paid'] as const)[Math.floor(Math.random() * 3)],
      })
