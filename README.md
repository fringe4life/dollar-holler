...
- **Authentication:** Better Auth 1.7 (`1.7.0-rc.4`) with email/password ([`src/lib/auth.server.ts`](./src/lib/auth.server.ts), Drizzle adapter `provider: "sqlite"`, `advanced.database.joins`, `allowedHosts` for `localhost`, `*.workers.dev`, and `*.pages.dev`, `@better-auth/drizzle-adapter` relations-v2); login/signup/forgot/reset/change-password via remote forms; logout via remote `command`; account rows keyed by `(issuer, account_id)` (credentials use `local:credential`)
