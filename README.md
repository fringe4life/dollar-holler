...
- Better Auth is configured in `auth.server.ts` as isolate-cached `getAuth()` so `drizzleAdapter` does not read `db._` at import; UUIDv7 (uuidv7 package) for user ID generation; session cookie cache enabled. After upgrading to 1.7.3, apply `0004_purple_fat_cobra` (drop `account_issuer_accountId_uidx`, then drop `issuer`) and `0005_left_squadron_sinister` (non-unique `(provider_id, account_id)` lookup index) under `src/lib/server/db/d1/`. Account identity is `(provider_id, account_id)`, matching 1.6.
