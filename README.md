...
- `account` - Auth provider accounts; identity is unique `(issuer, provider_account_id)` (email/password: `issuer` = `local:credential`, `provider_account_id` = user id); index `(user_id)` and unique `(issuer, provider_account_id)`
...
- **Modern Authentication:** Better Auth 1.7 with email/password; account identity scoped by `(issuer, provider_account_id)`
...
- Better Auth is configured in `auth.server.ts` to use UUIDv7 (uuidv7 package) for user ID generation and includes session caching for performance. After upgrading to 1.7 rc.2, run the account identity migration under `src/lib/server/db/migrations/` before signing in against an existing Turso database.
